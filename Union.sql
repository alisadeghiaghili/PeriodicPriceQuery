...
where Date.ID Between (select ID from Auction_DM.General_Dim.Date where PersianDate = @StartDate) And (select ID from Auction_DM.General_Dim.Date where PersianDate = @EndDate)
